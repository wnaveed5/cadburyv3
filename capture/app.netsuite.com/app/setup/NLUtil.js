...
function fireAlertWithTranslation(key, params, concatValue) {
	...
	if (NS.Translations) {
		alert(NS.Translations.getTranslatedString(key, params) + valueToConcat);
	} else {
		alert(parent.NS.Translations.getTranslatedString(key, params) + valueToConcat);
	}
}
...
function extract_date_time(str)
{
	...
	if (spaceIdx > 0) {
		...
		fireAlertWithTranslation('NLAlertContext.INVALID_DATETIME_VALUE_MISS_SPACES_BETWEEN_DATE_AND_TIME');
	}
...
function old_validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator, scale)
{
	...
	else if (type == "currency" || type == "currency2" || type == "poscurrency" || type == 'nonnegcurrency')
	{
		...
		{
			if (doalert)
				fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUES_MUST_BE_NUMBERS_UP_TO_1', { 1: '999,999,999,999,999.99' });
			validflag = false;
		}
		else if ( maxval != null && !isNaN(maxval) && Math.abs(numval)>=maxval )
		{
			if (doalert)
			{
				...
				fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUES_MUST_BE_NUMBERS_UP_TO_1', { 1: preDecimal + '.999999999999999' });
			}
		...
		{
			if (doalert) fireAlertWithTranslation('NLAlertContext.INVALID_CURRENCY_VALUE_VALUE_CAN_NOT_BE_NEGATIVE');
			validflag = false;
		}
...
function expandCollapseFieldGroup(id) {
	var fg = document.getElementById(id);
	var trBody = document.getElementById('tr_' + fg.id);
	var trOutsideAbove = document.getElementById('tr_above_' + fg.id);
	var trOutsideBelow = document.getElementById('tr_below_' + fg.id);
	var expanded = fg.getAttribute('aria-expanded') === 'true';
	fg.setAttribute('aria-expanded', !expanded);

	if (trBody)	trBody.setAttribute('aria-expanded', !expanded);
	if (trOutsideAbove)	trOutsideAbove.setAttribute('aria-expanded', !expanded);
	if (trOutsideBelow)	trOutsideBelow.setAttribute('aria-expanded', !expanded);
}
